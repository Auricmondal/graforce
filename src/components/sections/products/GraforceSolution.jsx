...
const SolutionCardData = [
  {
    ...
    icon: SolutionIcon,
    title: 'Plasma Power',
    description: 'High-voltage plasma (1300–1500 °C) cracks methane into hydrogen and carbon.',
  ...
  },
  {
    ...
    description: 'Up to 95% of the energy in methane is converted into hydrogen and carbon.',
  ...
  },
  {
    ...
    description: 'Plasmalyzer® maximizes energy output from every unit of methane processed.',
      ...
      <SectionWrapper sectionClassName='bg-cst-neutral-1' className='space-y-2'>
        ...
        <div className='grid grid-cols-1 md:grid-cols-1 gap-2'>
          {SolutionCardData.map((card) => (
            <GraforceSolutionCard key={card.id} icon={card.icon} title={card.title} description={card.description} onClick={card.onClick} />
          ))}
        </div>
      </SectionWrapper>
    </div>
