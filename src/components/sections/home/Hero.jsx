...
const Hero = () => {
  const CustomerAvatar = ({ src, className = "" }) => {
    return(
      <Image src={src} alt="Avatar" className={`w-10 h-10 rounded-full ${className}`} />
    )
  }
  ...
  return (
    <div>
      ...
      <div className="h-screen relative overflow-hidden bg-gradient-to-br from-white to-primary-300">
        <Image src={Path} alt="Path" className="absolute  -bottom-1/5 left-0 min-h-full object-cover object-right" />
        ...
        <FloatingTip className="top-100 left-1/7" delay={0.2}>• 0% CO₂ Costs</FloatingTip>
        <FloatingTip className="top-120 right-1/3" delay={0.6}>• 50% Energy Savings</FloatingTip>
