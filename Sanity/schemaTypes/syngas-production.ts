...
export default defineType({
  name: "syngas-production",
  title: "Syngas Production Page",
  type: "document",
  fields: [
    defineField({
      name: "language",
  ...
  preview: {
    select: {
      language: "language",
      title: "syngasHeroSection.title", // ✅ fixed (was methaneplasmalyzerHeroSection)
    },
    prepare(selection) {
      const { language, title } = selection;
      return {
        title: `Syngas Production Page [${language?.toUpperCase() || "??"}]`,
